...
class CartKerns:
    ...
    def interpolate_eigs(self):
        ...
        ITP_H   = scipy.interpolate.interp2d(self.raw_k, z, self.raw_H.T, 
                                             kind='linear', 
                                             bounds_error=False,fill_value=np.nan)
        ITP_V   = scipy.interpolate.interp2d(self.raw_k, z, self.raw_V.T, 
                                             kind='linear',
                                             bounds_error=False,fill_value=np.nan)
        
        ITP_H_   = scipy.interpolate.interp2d(self.raw_k_, z, self.raw_H_.T,
                                              kind='linear',
                                              bounds_error=False,fill_value=np.nan)
        ITP_V_   = scipy.interpolate.interp2d(self.raw_k_, z, self.raw_V_.T,
                                              kind='linear',
                                              bounds_error=False,fill_value=np.nan)
        
        ITP_dzH   = scipy.interpolate.interp2d(self.raw_k, z, self.raw_dzH.T,
                                               kind='linear',
                                               bounds_error=False,fill_value=np.nan)
        ITP_dzV   = scipy.interpolate.interp2d(self.raw_k, z, self.raw_dzV.T,
                                               kind='linear',
                                               bounds_error=False,fill_value=np.nan)

        ITP_dzH_   = scipy.interpolate.interp2d(self.raw_k_, z, self.raw_dzH_.T,
                                                kind='linear',
                                                bounds_error=False,fill_value=np.nan)
        ITP_dzV_   = scipy.interpolate.interp2d(self.raw_k_, z, self.raw_dzV_.T,
                                                kind='linear',
                                                bounds_error=False,fill_value=np.nan)
    ...
    def compute_kernels(self, q_vec_ind):
        ...
        for idx, (kxi, kyi) in enumerate(zip(kx_ind, ky_ind)):
            ...
            H_k  = self.ITP_H(abs_k, self.z).squeeze()
            V_k  = self.ITP_V(abs_k, self.z).squeeze()
            dzH_k = self.ITP_dzH(abs_k, self.z).squeeze()
            dzV_k = self.ITP_dzV(abs_k, self.z).squeeze()
                                    
            # constructing the interpolated eigenfunctions for k'
            H__k  = self.ITP_H_(abs_kp, self.z).squeeze()
            V__k  = self.ITP_V_(abs_kp, self.z).squeeze()
            dzH__k = self.ITP_dzH_(abs_kp, self.z).squeeze()
            dzV__k = self.ITP_dzV_(abs_kp, self.z).squeeze()
            ...
            Poloidal_kernel[idx] = self.flow_kern(cos_khk_h, cos_kq, )        
    ...
    def flow_kern(self):
        ...
        return (np.trapz(integrand_P*rho[None,:],z,axis=1))*LZ**2
